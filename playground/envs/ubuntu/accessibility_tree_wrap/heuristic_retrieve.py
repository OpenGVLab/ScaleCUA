...
sidebar_nodes = json.load(open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "sidebar_nodes.json"), "r"))
...
def is_interactive_node(node: ET, platform="ubuntu"):
    ...
    noninteractive_tags = ['heading', 'static', 'document', 'label', 'cell', 'text', 'icon', 'paragraph', 'section'] # FIXME: cell?
...
def is_wrong_node(node: ET, platform='ubuntu'):
    libreoffice_menu_names = ['File', 'Edit', 'View', 'Insert', 'Format', 'Tools', 'Window', 'Help', 'Styles', \
                              'Slide', 'Slide Show', \
                              'Sheet', 'Data', \
                              'Table', 'Form']
    tag = node.tag
...
def filter_nodes(root: ET, platform="ubuntu", check_image=False, mode='', overlap_ratio=1.0):
    filtered_nodes = []
    ...
    for node in root.iter():
        if judge_node(node, platform, check_image):
            ...
            if keep and 'nonoverlap' in mode:
                keep, filtered_nodes = judge_overlap_node(filtered_nodes, node, platform, overlap_ratio)
            if keep:
...
def get_clickable_elements(html_str, platform="ubuntu"):
    ...
    nodes = filter_nodes(root, platform="ubuntu", check_image=True, mode='nosidebar_interactive')
    ...
    for _node in nodes:
        ...
        if coords_str and size_str:
            try:
                ...
                clickable_elements.append({"id": index, "bbox": [coords[0], coords[1], bottom_right[0], bottom_right[1]], "text": node_text, "name": _node.get("name", "")})
...
def main(step_idx, root, filter_mode):
    ...
    with open(os.path.join(root, f"step_{step_idx}.png"), "rb") as f:
        screenshot = f.read()
    filtered_nodes = filter_nodes(tree.getroot(), platform='ubuntu', check_image=True, mode=filter_mode, overlap_ratio=1.0)
    marks, drew_nodes, element_list, tagged_screenshot = draw_bounding_boxes(filtered_nodes, screenshot)
