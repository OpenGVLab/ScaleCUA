...
from envs.ubuntu.accessibility_tree_wrap.heuristic_retrieve import (
...
MANAGER_LAUNCH_SCRIPT = (
    "mkdir -p log_env_api/env_manager;"
...
class UbuntuEnv(BaseEnv):
    def __init__(
        ...
        **kwargs,
        ...
        if self.server_path is None:
            ...
            script = MANAGER_LAUNCH_SCRIPT.format(
                port=self.manager_port,
                time=datetime.datetime.now().strftime("%Y%m%d%H%M%S"),
            )
            self.localhost_manager_process = subprocess.Popen(
                script, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            time.sleep(5)
            if self.localhost_manager_process.poll() is not None:
                raise Exception(
                    f"create env manager failed: {self.localhost_manager_process.poll()}"
                )
            self.server_path = "http://0.0.0.0"
            self.localhost_as_manager = True
            logger.info(
                f"create env manager locally success at port: {self.manager_port}, pid: {self.localhost_manager_process.pid}"
            )
        else:
        ...
        if self.env_port is None:
            ...
            response = self._request_handler(
                create_url, try_max_times=self.connect_max_try
            )
        ...
        response = self._request_handler(
            start_url, data, try_max_times=self.connect_max_try
        )
        logger.info("create env done")
        self.screen_size = [data["screen_width"], data["screen_height"]]

    def _request_handler(
        self, url, data=None, method="POST", try_max_times=5, timeout=60000
    ):
        """
        ...
        try:
            ...
            if self.localhost_as_manager:
                ...
                logger.info(
                    f"terminate localhost env manager done, freed port: {self.manager_port}"
                )
            self.close()
    ...
    def reset(self, **kwargs) -> Dict[str, Any]:
        ...
        response = self._request_handler(
            ...
            try_max_times=self.connect_max_try,
        )
    ...
    def exit(self):
        ...
        if self.use_api_manager:
            ...
            _ = self._request_handler(
                terminate_url, data, try_max_times=self.connect_max_try
            )
    ...
    def get_screenshot(self):
        ...
        response = self._request_handler(
            screenshot_url, method="GET", try_max_times=self.connect_max_try
        )
    ...
    def get_a11tree(self):
        ...
        response = self._request_handler(
            a11tree_url, method="GET", try_max_times=self.connect_max_try
        )
        ...
        if "evaluator" in self.task_config:
            if self.task_config["evaluator"]["func"] == "infeasible":
                ...
                if len(self.action_history) > 0 and last_action["name"] == "terminate":
                    if (
                        "status" in last_action["parameters"]
                        and last_action["parameters"]["status"] == "failure"
                    ):
                        return 1
                    else:
                        return 0
    ...
    def _execute_single_action(self, command: str):
        ...
        _ = self._request_handler(
            exec_url, {"command": command}, try_max_times=self.connect_max_try
        )
    ...
    def _execute_move_to(self, parameters: dict) -> bool:
        try:
            if "x" not in parameters or "y" not in parameters:
                raise KeyError(
                    f"moveTo requires 'x' and 'y' as parameter, current params: {parameters}"
                )
            x = parameters["x"]
            y = parameters["y"]
    ...
    def _execute_click(self, parameters: dict) -> bool:
        try:
            ...
            if button not in ["left", "right", "middle"]:
                raise Exception(
                    f"the button argument {button} is not one of 'left', 'middle', 'right'"
                )

            command = (
                f"pyautogui.click(button='{button}', x={x}, y={y}, clicks={clicks})"
            )
            self._execute_single_action(command)
            return True
        except Exception as e:
    ...
    def _execute_write(self, parameters: dict) -> bool:
        try:
            if "message" not in parameters:
                raise KeyError(
                    f"write requires 'message' as parameter, current params: {parameters}"
                )
    ...
    def _execute_press(self, parameters: dict) -> bool:
        try:
            if "keys" not in parameters:
                raise KeyError(
                    f"press requires 'keys' as parameter, current params: {parameters}"
                )
            ...
            if isinstance(keys, str):
                ...
                if keys not in KEYBOARD_KEYS:
                    logger.warning(
                        f"Key must be one of {KEYBOARD_KEYS}. Skip current key: {keys}"
                    )
                    return False
                ...
                for key in keys:
                    ...
                    if key not in KEYBOARD_KEYS:
                        logger.warning(
                            f"Key must be one of {KEYBOARD_KEYS}. Skip current key: {key}"
                        )
                        continue
                    for _ in range(presses):
                ...
                raise Exception(
                    f"'keys' for press must be type str or list, current keys are: {keys}"
                )
    ...
    def _execute_response(self, parameters: dict) -> bool:
        try:
            if "answer" not in parameters:
                raise KeyError(
                    f"response requires 'answer', current params: {parameters}"
                )
    ...
    def _execute_terminate(self, parameters: dict) -> bool:
        try:
            if "status" not in parameters:
                raise KeyError(
                    f"terminate requires 'status', current params: {parameters}"
                )
    ...
    def _execute_double_click(self, parameters: dict) -> bool:
        try:
            ...
            if clicks != 2:
                logger.warning(
                    f"doubleClick suggests 'clicks' set to 2, current is: {clicks}, ignored"
                )
    ...
    def _execute_scroll(self, parameters: dict) -> bool:
        try:
            if "clicks" not in parameters:
                raise KeyError(
                    f"scroll requires 'clicks', current params: {parameters}"
                )
    ...
    def start_recording(self):
        ...
        _ = self._request_handler(
            start_recording_url, method="GET", try_max_times=self.connect_max_try
        )
...
if __name__ == "__main__":
    action_1 = [
        {"name": "write", "parameters": {"message": "cd large_files && ls -R"}},
        {"name": "press", "parameters": {"keys": "enter", "presses": 1}},
    ]
    action_2 = [
        {"name": "terminate", "parameters": {"status": "success", "answer": None}}
    ]
