...
def get_active_tab_info(env, config: Dict[str, str]):
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(active_tab_url, timeout=90000)
            page.wait_for_load_state('load')  # Wait for the 'load' event to complete
            active_tab_info = {
                'title': page.title(),
                'url': page.url,
...
def get_pdf_from_url(env, config: Dict[str, str]) -> str:
    ...
    _url = config["path"]
    _path = os.path.join(env.cache_dir, config["dest"])
    # pdf from url is usually used for expected result, so we can cache it
    ...
    with sync_playwright() as p:
        ...
        page = browser.new_page()
        page.goto(_url, timeout=60000)  # Set timeout to 60 seconds (60000 milliseconds)
        page.pdf(path=_path)
        browser.close()
...
def get_googledrive_file(env, config: Dict[str, Any]) -> str:
    ...
    @args: keys in config dict
        settings_file(str): target filepath to the settings file for Google Drive authentication, default is 'config/tasks/osworld/settings/googledrive/settings.yml'
    ...
    """
    settings_file = config.get('settings_file', 'config/tasks/osworld/settings/googledrive/settings.yml')
    ...
    with sync_playwright() as p:
        ...
        for context in browser.contexts:
            for page in context.pages:
                ...
                if unquote(page.url).strip('/') == unquote(active_tab_url).strip('/'):
                    target_page = page
