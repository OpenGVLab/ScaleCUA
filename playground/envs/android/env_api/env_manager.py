...
class EnvManager:
    ...
    def get_screenshot(
        ...
        if prefix != None:
            page_executor.update_screenshot(
                prefix=str(env.get("step")) + "_" + prefix, suffix=suffix
            )
        else:
    ...
    def get_xml(self, env_id: str):
        ...
        if not ac_status:
            xml_status = controller.get_xml(
                prefix=str(env.get("step")), save_dir=config.xml_dir
            )
            print(xml_status)
            if "ERROR" in xml_status:
                ...
                if "ERROR" in xml_status:
                    ...
                    xml_path = os.path.join(
                        config.xml_dir, str(env.get("step")) + ".xml"
                    )
            else:
