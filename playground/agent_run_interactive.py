...
def run_agent(agent, instruction: str, env: None, root_folder:None, **task_kwargs):
    ...
    for idx in range(20):
        ...
        if idx == 21:
            ...
            info, code = agent.predict(instruction=instruction, observation=obs, env=env)

        draw_tag_on_image(code[0], obs["screenshot"], os.path.join(save_folder, f"draw_step-{idx}.png"))
        with open(os.path.join(save_folder, 'action.jsonl'),'a', encoding='utf-8') as action_save:
...
def main():
    ...
    if 'ui_grounding_model' in agent_config:
        ...
        if 'grounding_height' in engine_params_for_ui_grounding and engine_params_for_ui_grounding['grounding_height'] is None:
            engine_params_for_ui_grounding['grounding_height'] = screen_height * engine_params_for_ui_grounding['grounding_width'] / screen_width
