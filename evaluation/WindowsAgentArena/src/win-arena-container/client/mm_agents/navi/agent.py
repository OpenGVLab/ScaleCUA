...
from mm_agents.navi.prompts import SYS_PROMPT_IN_SCREENSHOT_OUT_CODE
import copy
...
def resize_image_openai(image):
    ...
    if (
        out_w <= max_size
        and out_h <= max_size
        and min(out_w, out_h) <= target_short_side
    ):
...
class NaviAgent:
    def __init__(
        self,
        server: str = "azure",
        model: str = "gpt-4o-2024-11-20",  # openai or "phi3-v"
        som_config=None,
        ...
        if som_origin in ["oss", "mixed-oss"]:  # oss extractor
            ...
            from mm_agents.navi.screenparsing_oss.omniparser.omniparser import (
                Omniparser,
            )
        ...
        if model == "phi3-v":
            ...
            self.gpt4v_planner = Phi3_Planner(
                server="azure", model="phi3-v", temperature=temperature
            )
        else:
            self.gpt4v_planner = GPT4V_Planner(
                server=self.server, model=self.model, temperature=temperature
            )
            if use_last_screen:
                # self.gpt4v_planner.system_prompt = planner_messages.planning_system_message_shortened_previmg
                self.gpt4v_planner.system_prompt = SYS_PROMPT_IN_SCREENSHOT_OUT_CODE
...
"""
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        window_title, window_names_str, window_rect, computer_clipboard = (
            obs["window_title"],
        ...
        if self.auto_window_maximize:
            ...
            if "System tray" not in window_title and "Defender" not in window_title:
                if (
                    window_title != self.prev_window_title
                    and window_rect != self.prev_window_rect
                ):
                    # debug logging {{{
        ...
        if not override_plan:
            ...
            if self.som_origin == "a11y":
                ...
                rects = [
                    [
                        int(ent["shape"]["x"]),
                        int(ent["shape"]["y"]),
                        int((ent["shape"]["x"] + ent["shape"]["width"])),
                        int((ent["shape"]["y"] + ent["shape"]["height"])),
                    ]
                    for ent in regions
                ]
                color_mapping_debug = {"a11y": "red"}
                color_mapping_prompt = {"a11y": "red"}
                image_debug, image_prompt, list_of_text = self.parser_to_prompt(
                    image, regions, color_mapping_debug, color_mapping_prompt
                )  # full set-of-marks drawing w/ visibility filtering, overlap detection, and colors
                logs["foreground_window_regions"] = image_debug

            elif (
                self.som_origin == "mixed"
            ):  # TODO: combine a11y and internal extractorsin a cleaner way
                # combine both a11y and internal extractors

                # a11y extractor
                from mm_agents.navi.a11y_demo import (
                    propose_ents as get_a11y_ents,
                    get_mask_from_entities,
                    filter_entities_with_mask,
                    detections_to_entities,
                )
                ...
                list_of_text = self.extractor.create_text_list(
                    detected_regions, image.width, image.height
                )
                ...
                try:
                    image_debug_ex = draw_colored_image(
                        image_debug_ex,
                        detected_regions,
                        color_mapping_debug,
                        draw_numbers=True,
                ...
                try:
                    image_debug_ex, _, models_debug_txt = self.parser_to_prompt(
                        image, regions_ex, color_mapping_debug, color_mapping_debug
                    )
                ...
                from mm_agents.navi.a11y_demo import (
                    ...
                    filter_entities_with_mask,
                    detections_to_entities,
                    filter_nonvis_and_oob_entities,
                )
                from mm_agents.navi.screenparsing_oss.utils.som import filter_entities
                import numpy as np
                ...
                regions_a11y = filter_nonvis_and_oob_entities(
                    regions_a11y, image.width, image.height
                )
                ...
                # omni extractor
                rendering = "N/A"
                regions_omni = self.omni_proposal.propose_ents(
                    image, with_captions=False
                )
                ...
                try:
                    logs["parsed_content_icon"] = self.omni_proposal.caption_ents(
                        image, regions
                    )
                except Exception as e:
            ...
            image_resized, w_resized, h_resized, factor = resize_image_openai(
                view_image
            )
            image_prompt_resized, w_resized, h_resized, factor = resize_image_openai(
                image_prompt
            )
            ...
            if self.use_last_screen:
                last_image = (
                    self.last_image if self.last_image is not None else image_resized
                )
