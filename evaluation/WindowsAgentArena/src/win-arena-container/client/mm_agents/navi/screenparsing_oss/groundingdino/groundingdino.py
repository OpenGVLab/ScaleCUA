from mm_agents.navi.screenparsing_oss.element_extractor.utils import (
    crop_image,
    draw_multiple_bboxes,
)
from mm_agents.navi.screenparsing_oss.groundingdino.icon_localization import (
    load_dino_model,
    det,
)
import os
...
class GroundingDino:
    def __init__(
        self, prompts=["icon"], config_path=None, weights_path=None, device=None
    ):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.prompts = prompts

        config_path = config_path or os.path.join(
            os.path.dirname(__file__), "config/GroundingDINO_SwinT_OGC.py"
        )
        weights_path = (
            weights_path or "/models/groundingDINO/groundingdino_swint_ogc.pth"
        )
        ...
        self.groundingdino_model = load_dino_model(
            config_path, weights_path, device=self.device
        ).eval()
    ...
    def propose_ents(self, image_pil):
        def det2ent(prompt, coords):
            ...
            return {"from": "groundingdino", "type": prompt, "shape": shape}
