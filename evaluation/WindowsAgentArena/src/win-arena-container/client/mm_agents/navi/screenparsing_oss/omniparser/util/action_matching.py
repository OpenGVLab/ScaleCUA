...
def action_2_format(step_data):
    # transform the action data from the test dataset to the format used for calculating matching score
    action_type = step_data["action_type_id"]

    if action_type == 4:
        if step_data["action_type_text"] == 'click':  # click
            touch_point = step_data["touch"]
            lift_point = step_data["lift"]
        else:  # Scroll down/ up/ left/ right
...
def pred_2_format_simplified(step_data):
    # Transform the model output to the format used for calculating action matching
    action_type = step_data["action_type"]

    if action_type == 'click' :  # click
        action_type_new = 4
        touch_point = step_data["click_point"]
        lift_point = step_data["click_point"]
        typed_text = ""
