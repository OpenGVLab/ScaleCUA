from mm_agents.navi.screenparsing_oss.utils.som import (
    draw_som,
    filter_entities,
    add_labels,
)
...
def create_text_list(entities: List[dict], width, height) -> str:
    ...
    for ent in entities:
        ...
        shape = ent["shape"]
        location = [
            round(shape["x"] / width, 2),
            round(shape["y"] / height, 2),
            round((shape["x"] + shape["width"]) / width, 2),
            round((shape["y"] + shape["height"]) / height, 2),
        ]
        # check if the key exists
        if "text" in ent.keys():
            list_of_text += (
                f"{region_id} | {region_type} | {ent['text']} | {location}\n"
            )
        else:
...
def parser_to_prompt(
    screen, entities, color_mapping_debug=None, color_mapping_prompt=None
):
    ...
    image_som_notext = draw_som(
        screen, vis_entities, color_mapping=color_mapping_prompt
    )
