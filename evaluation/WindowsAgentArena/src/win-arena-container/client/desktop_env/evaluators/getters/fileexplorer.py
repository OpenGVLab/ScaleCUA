...
def get_is_details_view(env, config):
    if not config["folder_path"]:
        return False
    is_details_view = env.controller.get_vm_file_explorer_is_details_view(
        config["folder_path"]
    )
...
def get_is_all_docx_in_archive(env, config) -> str:
    ...
    folder_exists = env.controller.execute_python_command(
        f"import os; print(os.path.exists('{folder_path}'))"
    )
    ...
    archive_exists = env.controller.execute_python_command(
        f"import os; print(os.path.exists('{archive_path}'))"
    )
...
def get_is_directory_read_only_for_user(env, config) -> bool:
    ...
    return env.controller.get_vm_is_directory_read_only_for_user(
        config["directory"], config["user"]
    )
...
def get_are_all_images_tagged(env, config) -> bool:
    ...
    return env.controller.get_vm_are_all_images_tagged(
        config["directory"], config["tag"]
    )
...
def get_is_files_moved_downloads(env, config) -> str:
    ...
    downloads_directory = json.dumps(
        env.controller.get_vm_directory_tree("C:\\Users\\Docker\\Downloads")
    )
    old_downloads_directory = env.controller.get_vm_directory_tree(
        "C:\\Users\\Docker\\OldDownloads"
    )
