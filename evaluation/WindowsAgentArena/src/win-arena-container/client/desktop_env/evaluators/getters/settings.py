...
def get_night_light_state(env, config: Dict[str, str]):
    ...
    if decoded_result[24] == "1":
        ...
        if starting_hour != 202:
            starting_minute = int(
                decoded_result[30] if decoded_result[29] != "0" else "0"
            )
            ...
            ending_minute = int(
                decoded_result[ending_index + 2]
                if decoded_result[ending_index + 1] != "0"
                else "0"
            )
...
def get_storage_sense_run_frequency(env, config: Dict[str, str]):
    ...
    is_storage_sense_enabled = env.controller.get_registry_key(
        KEY_PATH, STORAGE_SENSE_ON_OFF_SETTING_PATH
    )
    if (
        is_storage_sense_enabled["status"] == "success"
        and is_storage_sense_enabled["output"] == "1\n"
    ):
        storage_sense_run_frequency = env.controller.get_registry_key(
            KEY_PATH, STORAGE_SENSE_FREQUENCY_SETTING_PATH
        )
...
def get_active_hours_of_user_to_not_interrupt_for_windows_updates(
    env, config: Dict[str, str]
):
    """
    Decoder for the registry key
    ...
    active_start_hours = env.controller.get_registry_key(
        KEY_PATH, ACTIVE_HOURS_START_SETTING_PATH
    )
    active_end_hours = env.controller.get_registry_key(
        KEY_PATH, ACTIVE_HOURS_END_SETTING_PATH
    )

    active_start_hours_formatted = generate_time(
        int(active_start_hours["output"].split("\n")[0]), 0
    )
    active_end_hours_formatted = generate_time(
        int(active_end_hours["output"].split("\n")[0]), 0
    )

    if (
        active_start_hours["status"] == "success"
        and active_end_hours["status"] == "success"
    ):
