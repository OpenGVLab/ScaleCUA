...
def find_red_circle(image) -> bool:
    ...
    distances = np.sqrt(
        (red_pixels[1] - centroid_x) ** 2 + (red_pixels[0] - centroid_y) ** 2
    )
...
def get_image_dimension_matches_input(env, config: Dict[str, str]) -> bool:
    ...
    file_localhost_path = get_vm_file(
        env, {"path": filepath, "dest": os.path.split(filepath)[-1]}
    )
