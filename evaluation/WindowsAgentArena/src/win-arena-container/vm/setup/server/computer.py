...
class Mouse:
    ...
    def move_id(self, id):
        ...
        screen_x, screen_y = screen_utils.image_to_screen_coordinates(
            center_x, center_y, self.window_rect
        )
    ...
    def is_installed(self, program):
        try:
            ...
            registry_key = winreg.OpenKey(
                ...
                f"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\{program}.exe",
            )
        ...
        try:
            ...
            registry_key = winreg.OpenKey(
                winreg.HKEY_LOCAL_MACHINE,
                f"SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\{program}.exe",
            )
            executable, _ = winreg.QueryValueEx(registry_key, "")
            winreg.CloseKey(registry_key)
            if executable:
...
class WindowManager:
    ...
    def switch_to_application(self, app_name):
        ...
        closest_match = difflib.get_close_matches(
            app_name, [self.normalize_app_name(a) for a in self.applications], n=1
        )
        ...
        for window in gw.getAllWindows():
            if (
                self.is_window_visible(window._hWnd)
                and self.normalize_app_name(self.get_app_name(window._hWnd)) == app_name
            ):
    ...
    def switch_to_application1(self, app_name):
        ...
        for window in gw.getAllWindows():
            if (
                self.is_window_visible(window._hWnd)
                and self.get_app_name(window._hWnd) == app_name
            ):
                window.restore()  # Restore the window if it's minimized
                window.maximize()  # Maximize the window
...
def main():
    ...
    computer = Computer(
        rects, window_rect, screenshot_img, scale=(1.0, 1.0), swap_ctrl_alt=True
    )
