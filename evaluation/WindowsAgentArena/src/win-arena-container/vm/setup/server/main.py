...
def probe_endpoint():
    ...
    return (
        jsonify({"status": "Probe successful", "message": "Service is operational"}),
        200,
    )
...
def shutdown_endpoint():
    ...
    return (
        jsonify(
            {
                "status": "Shutdown started successfully",
        ...
        if user_platform == "Linux":
            ...
            if _has_active_terminal(desktop):
                ...
                terminals: List[_Element] = desktop_xml.xpath(
                    xpath, namespaces=_accessibility_ns_map
                )
...
def obs_winagent():
    ...
    if window_title == "Program Manager" or window_title == "":
        ...
        rect = win32gui.GetWindowRect(
            window
        )  # x0, y0, x1, y1 from the screen in global coordinates
...
def get_obs_winagent():
    try:
        image, window_title, rect, window_names_str, computer_clipboard, human_input = (
            obs_winagent()
        )
...
def _create_atspi_node(
    ...
    for attrbt in attributes:
        ...
        if len(attribute_name) == 0:
            continue
        attribute_dict[
            "{{{:}}}{:}".format(_accessibility_ns_map["attr"], attribute_name)
        ] = attribute_value
    #  }}} Attributes #
    ...
    try:
        ...
        attribute_dict["{{{:}}}size".format(_accessibility_ns_map["cp"])] = str(
            component.getSize()
        )
    #  }}} Component #
    ...
    try:
        ...
        attribute_dict["{{{:}}}locale".format(_accessibility_ns_map["doc"])] = (
            document.getLocale()
        )
        attribute_dict["{{{:}}}pagecount".format(_accessibility_ns_map["doc"])] = str(
            document.getPageCount()
        )
        attribute_dict["{{{:}}}currentpage".format(_accessibility_ns_map["doc"])] = str(
        ...
        for attrbt in document.getAttributes():
            ...
            if len(attribute_name) == 0:
                continue
            attribute_dict[
                "{{{:}}}{:}".format(_accessibility_ns_map["docattr"], attribute_name)
            ] = attribute_value
    #  }}} Document #
    ...
    try:
        ...
        try:
            attribute_dict["{{{:}}}value".format(_accessibility_ns_map["val"])] = str(
                value.currentValue
            )
        except:
            pass
        try:
            attribute_dict["{{{:}}}min".format(_accessibility_ns_map["val"])] = str(
                value.minimumValue
            )
        except:
            pass
        try:
            attribute_dict["{{{:}}}max".format(_accessibility_ns_map["val"])] = str(
                value.maximumValue
            )
        except:
            pass
        try:
            attribute_dict["{{{:}}}step".format(_accessibility_ns_map["val"])] = str(
    ...
    try:
        ...
        for i in range(action.nActions):
            ...
            attribute_dict[
                "{{{:}}}{:}_desc".format(_accessibility_ns_map["act"], action_name)
            ] = action.getDescription(i)
            attribute_dict[
                "{{{:}}}{:}_kb".format(_accessibility_ns_map["act"], action_name)
    ...
    xml_node = lxml.etree.Element(
        node_role_name, attrib=attribute_dict, nsmap=_accessibility_ns_map
    )
    ...
    if flag == "calc" and node_role_name == "table":
        ...
        version_str: str = subprocess.run(
...
def _create_pywinauto_node(
    node: BaseWrapper, depth: int = 0, flag: Optional[str] = None
) -> _Element:
    ...
    #  States {{{ #
    try:
        attribute_dict["{{{:}}}enabled".format(_accessibility_ns_map["st"])] = str(
            node.is_enabled()
        ).lower()
    except:
        pass
    try:
        attribute_dict["{{{:}}}visible".format(_accessibility_ns_map["st"])] = str(
            node.is_visible()
        ).lower()
    except:
        pass
    try:
        attribute_dict["{{{:}}}active".format(_accessibility_ns_map["st"])] = str(
            node.is_active()
        ).lower()
    except:
        pass
    ...
    if hasattr(node, "is_minimized"):
        try:
            attribute_dict["{{{:}}}minimized".format(_accessibility_ns_map["st"])] = (
                str(node.is_minimized()).lower()
            )
        except:
            pass
    if hasattr(node, "is_maximized"):
        try:
            attribute_dict["{{{:}}}maximized".format(_accessibility_ns_map["st"])] = (
                str(node.is_maximized()).lower()
            )
        except:
            pass
    if hasattr(node, "is_normal"):
        try:
            attribute_dict["{{{:}}}normal".format(_accessibility_ns_map["st"])] = str(
                node.is_normal()
            ).lower()
        except:
            pass
    ...
    if hasattr(node, "is_unicode"):
        try:
            attribute_dict["{{{:}}}unicode".format(_accessibility_ns_map["st"])] = str(
                node.is_unicode()
            ).lower()
        except:
            pass
    ...
    if hasattr(node, "is_collapsed"):
        try:
            attribute_dict["{{{:}}}collapsed".format(_accessibility_ns_map["st"])] = (
                str(node.is_collapsed()).lower()
            )
        except:
            pass
    if hasattr(node, "is_checkable"):
        try:
            attribute_dict["{{{:}}}checkable".format(_accessibility_ns_map["st"])] = (
                str(node.is_checkable()).lower()
            )
        except:
            pass
    if hasattr(node, "is_checked"):
        try:
            attribute_dict["{{{:}}}checked".format(_accessibility_ns_map["st"])] = str(
                node.is_checked()
            ).lower()
        except:
            pass
    if hasattr(node, "is_focused"):
        try:
            attribute_dict["{{{:}}}focused".format(_accessibility_ns_map["st"])] = str(
                node.is_focused()
            ).lower()
        except:
            pass
    if hasattr(node, "is_keyboard_focused"):
        try:
            attribute_dict[
                "{{{:}}}keyboard_focused".format(_accessibility_ns_map["st"])
            ] = str(node.is_keyboard_focused()).lower()
        except:
            pass
    if hasattr(node, "is_selected"):
        try:
            attribute_dict["{{{:}}}selected".format(_accessibility_ns_map["st"])] = str(
                node.is_selected()
            ).lower()
        except:
            pass
    if hasattr(node, "is_selection_required"):
        try:
            attribute_dict[
                "{{{:}}}selection_required".format(_accessibility_ns_map["st"])
            ] = str(node.is_selection_required()).lower()
        except:
            pass
    if hasattr(node, "is_pressable"):
        try:
            attribute_dict["{{{:}}}pressable".format(_accessibility_ns_map["st"])] = (
                str(node.is_pressable()).lower()
            )
        except:
            pass
    if hasattr(node, "is_pressed"):
        try:
            attribute_dict["{{{:}}}pressed".format(_accessibility_ns_map["st"])] = str(
                node.is_pressed()
            ).lower()
        except:
            pass
    ...
    if hasattr(node, "is_expanded"):
        try:
            attribute_dict["{{{:}}}expanded".format(_accessibility_ns_map["st"])] = str(
                node.is_expanded()
            ).lower()
        except:
            pass
    if hasattr(node, "is_editable"):
        try:
            attribute_dict["{{{:}}}editable".format(_accessibility_ns_map["st"])] = str(
                node.is_editable()
            ).lower()
        except:
            pass
    ...
    attribute_dict["{{{:}}}size".format(_accessibility_ns_map["cp"])] = (
    ...
    attribute_dict["{{{:}}}class".format(_accessibility_ns_map["win"])] = str(
        type(node)
    )
    ...
    node_role_name = "".join(
        map(
            lambda ch: ch if ch.isidentifier() or ch in {"-"} or ch.isalnum() else "-",
            node_role_name,
        )
    )
...
def get_accessibility_tree():
    ...
    if os_name == "Linux":
        ...
        for wnd in desktop.windows():
            logger.debug(
                "Win UIA AT parsing: %s(%d)", wnd.element_info.name, len(wnd.children())
            )
...
def get_documents_path():
    ...
    if documents_path and os.path.exists(documents_path):
        ...
        return (
            jsonify(error="Unsupported operating system or documents path not found"),
            400,
        )
...
def recycle_file():
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "message": f"{data['path']} recycled"}),
            200,
        )
    except:
        return (
            jsonify(
                {"status": "error", "message": f"Failed to recycle {data['path']}"}
            ),
            400,
        )
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        return (
            jsonify(
                {
                    "status": "success",
                    "message": f"Folder {folder_name} exists in {path}.",
                }
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"Folder {folder_name} does not exist in {path}.",
                }
            ),
            400,
        )
...
def get_file_exists():
    ...
    if os.path.exists(file_path) and os.path.isfile(file_path):
        return (
            jsonify(
                {"status": "success", "message": f"File {file_name} exists in {path}."}
            ),
            200,
        )
    else:
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"File {file_name} does not exist in {path}.",
                }
            ),
            400,
        )
    ...
    if is_details:
        return (
            jsonify(
                {
                    "status": "success",
                    "message": "File Explorer is in Details view",
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "File Explorer is not in Details view",
...
def activate_window():
    ...
    if os_name == "Windows":
        ...
        if by_class_name:
            return (
                "Get window by class name is currently not supported on Windows.",
                500,
            )
...
def clear_task_files():
    ...
    if failed_files and rmed_files:
        return (
            f"Failed to remove ({failed_files}) but sucessfully cleared ({rmed_files}) from {repr(path_to_rm)}.",
            200,
        )
    ...
    if not failed_files and rmed_files:
        return (
            f"All task files ({rmed_files}) cleared successfully from {repr(path_to_rm)}.",
            200,
        )
...
def start_recording():
    ...
    if platform_name == "Linux":
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Recording is not supported on this platform.",
                }
            ),
            400,
        )
    ...
    return jsonify(
        {
            "status": "success",
            "message": f"Started recording to: {recording_path}.\nCOMMAND: {start_command}",
        }
    )
...
def end_recording():
    ...
    if os.path.exists(recording_path):
        return (
            jsonify(
                {"status": "success", "message": f"record saved to: {recording_path}"}
            ),
            200,
        )
    else:
...
def save_state():
    ...
    if "snapshot_name" in request.form:
        snapshot_name = os.path.expandvars(
            os.path.expanduser(request.form["file_path"])
        )
    else:
...
def get_are_files_sorted_by_modified_time():
    ...
    if (
        explorer_sorted_files == sorted_items_names_desc
        or explorer_sorted_files == sorted_items_names_asc
    ):
        return (
            jsonify(
                {
                    "status": "success",
                    "message": f"{directory}: files are sorted by modified time",
                }
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"{directory}: files are not sorted by modified time",
                }
            ),
            400,
        )
...
def is_directory_read_only_for_user():
    ...
    if fileexplorer_utils.is_directory_read_only_for_user(directory, user):
        return (
            jsonify(
                {"status": "success", "message": f"{directory} is read-only for {user}"}
            ),
            200,
        )
    else:
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"{directory} is not read-only for {user}",
                }
            ),
            400,
        )
...
def are_all_images_tagged():
    ...
    image_files = [
        file for file in files if file.lower().endswith(("jpg", "jpeg", "tiff"))
    ]
    import fileexplorer_utils
        ...
        if not fileexplorer_utils.check_tag_in_image(image_path, tag):
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"{image_file} in {directory} is not tagged with {tag}",
                    }
                ),
                400,
            )
    return (
        jsonify(
            {
                "status": "success",
                "message": f"All images in {directory} are tagged with {tag}",
            }
        ),
        200,
    )
...
def get_check_if_timer_started():
    ...
    print(
        "Checking if timer exists for {} hours, {} minutes, and {} seconds".format(
    ...
    timer_exists = uiautomation_utils.clock_check_if_timer_started(
        hours, minutes, seconds
    )
...
def get_check_if_world_clock_exists():
    ...
    world_clock_exists = uiautomation_utils.clock_check_if_world_clock_exists(
        city, country
    )
    ...
    if world_clock_exists:
        ...
        return (
            jsonify({"status": "error", "message": "World clock does not exist"}),
            400,
        )
