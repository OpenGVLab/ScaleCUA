...
def load_args_as_dict():
    parser = argparse.ArgumentParser(
        description="Script to create a Windows Arena run for benchmarking on Azure ML."
    )
    parser.add_argument(
        "--ci_startup_script_path",
        default="Users/rbonatti/compute-instance-startup.sh",
        help="Path to the startup script for the compute instance (default: Users/rbonatti/compute-instance-startup.sh)",
    )
    parser.add_argument("--agent", default="navi", help="Agent to use (default: navi)")
    parser.add_argument(
        "--datastore_input_path",
        default="storage",
        help="Datastore input path (default: storage)",
    )
    parser.add_argument(
        "--docker_img_name",
        default="windowsarena/winarena:latest",
        help="Docker image name (default: winarena)",
    )
    parser.add_argument(
        "--exp_name", default="exp0", help="Experiment name (default: exp0)"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Use Managed Identity (default: False)",
    )
    parser.add_argument(
        "--json_name",
        default="evaluation_examples_windows/test_all.json",
        help="Name of the JSON file (default: evaluation_examples_windows/test_all.json)",
    )
    parser.add_argument(
        "--model_name", default="gpt-4o-mini", help="Model name (default: gpt-4o-mini)"
    )  # gpt-4o-mini or gpt-4-vision-preview or gpt-4o or gpt-4-1106-vision-preview
    parser.add_argument(
        "--som_origin", default="oss", help="Origin of the SOM (default: internal)"
    )  # internal or oss or a11y or mixed
    parser.add_argument(
        "--a11y_backend",
        default="uia",
        help="Type of acc tree. uia more precise, win32 faster",
    )  # uia (slower) or win32 (faster)
    args, _ = parser.parse_known_args()
    return vars(args)


def launch_vm_and_job(
    worker_id,
    exp_name,
    ...
    startup_script_ref = ScriptReference(
        path=ci_startup_script_path, timeout_minutes=10
    )
    ...
    try:
        ...
        logging.info(
            f"Compute instance status: {compute_instance.state}"
        ...
        if compute_instance.state != "Running":
            ...
            logging.info(
                f"Compute instance {compute_instance_name} is already {compute_instance.state}."
            )
    except:
        ...
        if use_managed_identity:
            identity_config = ManagedIdentityConfiguration(
                ...
                + azure_config["AZURE_ML_USER_ASSIGNED_IDENTITY"],
            ...
            identity = IdentityConfiguration(
                type="UserAssigned", user_assigned_identities=[identity_config]
            )
    ...
    if "OPENAI_API_KEY" in azure_config:
        ...
        raise KeyError(
            "Either 'OPENAI_API_KEY' must be available or both 'AZURE_API_KEY' and 'AZURE_ENDPOINT' must be available."
        )
...
def launch_experiment(config):
    ...
    for i in range(config["num_workers"]):
        p = Process(
            target=launch_vm_and_job,
            args=(
                i,
                config["exp_name"],
                docker_config,
                config["datastore_input_path"],
                config["num_workers"],
                config["agent"],
                azure_config,
                config["docker_img_name"],
                config["ci_startup_script_path"],
...
def launch_batch(experiments):
    for exp_name, config in experiments.items():
        if config.get("_done", False) or "_stop_time" in config:
            logging.info(
                f"Skipping experiment: {exp_name} as it has already been completed."
            )
            continue
        ...
        if "_start_time" in config and "_stop_time" not in config:
            user_input = (
                input(
                    f"Experiment '{exp_name}' was already started. Do you want to continue? (yes/no/skip): "
                )
                .strip()
                .lower()
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiments_json", help="Path to JSON file containing multiple experiments"
    )
    parser.add_argument(
        "--update_json",
        action="store_true",
        help="Update the experiments JSON with provided arguments",
    )
    args, _ = parser.parse_known_args()
    ...
    if args.experiments_json and args.update_json:
        try:
            ...
            existing_config = {
                k: v
                for k, v in experiments.get(exp_name, {}).items()
                if k.startswith("_")
            }
