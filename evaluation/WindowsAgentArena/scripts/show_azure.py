...
def get_results_from_json(result_dir, config, output_file):
    ...
    for exp_name, exp_details in tqdm(config.items(), desc="Processing Experiments"):
        ...
        row["uia_value"] = (
            "✅" if exp_details.get("a11y_backend", "") == "uia" else "❌"
        )
        ...
        for domain in [
            ...
            if os.path.isdir(domain_path):
                ...
                if task_results:
                    ...
                    row[domain] = (
                        sum(valid_results) / len(valid_results) * 100
                        if valid_results
                        else None
                    )
                else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate results table from JSON config."
    )
    parser.add_argument(
        "--result_dir",
        type=str,
    ...
    parser.add_argument(
        "--json_config", type=str, required=True, help="Path to JSON config file."
    )
    parser.add_argument(
        "--output_file",
        type=str,
