...
def main():
    ...
    with open(
        os.path.join(result_dir, f"output_{exp_name}_worker_{str(worker_id)}.txt"), "w"
    ) as f:
    ...
    os.system(
        "/entry_setup.sh"
    )  # since it's in root we can just do /script.sh and don't need cd /

    # launches the client script
    os.system(
        f"cd /client && python run.py --agent_name {agent} --worker_id {worker_id} --num_workers {num_workers} --result_dir {result_dir} --test_all_meta_path {json_name} --model {model_name} --som_origin {som_origin} --a11y_backend {a11y_backend}"
    )
