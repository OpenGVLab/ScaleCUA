...
DOCKER_RUN_SCRIPT_PATH = (
    script_dir / "docker" / "start_container_wsl.sh"
)  # Startup script for new container
...
def docker_run_container(
    container_name, platform="wsl", docker_name="evalkit_macos_auto", port=50922
):
    ...
    if container_exists(container_name):
        ...
        subprocess.run(
            ["bash", str(run_script_path), docker_name, str(port)], check=True
        )
