# Modified based on https://github.com/xlang-ai/OSWorld/blob/main/mm_agents/uitars_agent.py
...
def parsing_response_to_pyautogui_code(
    ...
    for response_id, response in enumerate(responses):
        ...
        if action_type == "hotkey":
            ...
            # Simulate typing action using pyautogui only
            ...
            if content:
                pyautogui_code += f"\nkeyboard.write('{stripped_content}', delay=0.1)"
                pyautogui_code += f"\ntime.sleep(0.5)\n"
                if content.endswith("\n") or content.endswith("\\n"):
...
class UITARSAgent:
    def __init__(
        ...
        if self.url is not None:
            ...
            if model == "uitars72bdpo":
                self.model_type = "qwen2vl"
                self.vlm = OpenAI(
                    base_url="http://10.140.66.5:8003/v1",
                    api_key="empty",
                )
                # should replace with your UI-TARS server api
            elif model == "ui_tars_15_7b":
                self.model_type = "qwen25vl"
                self.vlm = OpenAI(
                    base_url="http://10.140.66.134:8000/v1",
                    api_key="empty",
                )
            else:
                raise ValueError("Unspported UI-tars model")
