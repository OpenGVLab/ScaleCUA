...
class UIXMLTree:
    ...
    def process(
        ...
        self.xml_dict = xmltodict.parse(
            etree.tostring(self.root, encoding="utf-8"), attr_prefix=""
        )
        ...
        if "json" == str_type:
            ...
            return (
                json.dumps(self.xml_dict, indent=4, ensure_ascii=False)
                .replace(": {},", "")
                .replace(": {}", "")
            )
    ...
    def find_smallest_enclosing_node(self, root, bounds):
        smallest_node = None
        if "bounds" in root.attrib and check_bounds_containing(
    ...
    def should_remove_node(self, node):
        ...
        if parent is not None and "bounds" in parent.attrib:
            if not check_bounds_containing(
                node.attrib["bounds"], parent.attrib["bounds"]
            ):
        ...
        if node.tag == "node":
            if self.should_remove_node(node):
                ...
                if node.attrib["class"] != "":
                    if node.attrib["text"] != "":
                        node.attrib["name"] = (
                            get_words_in_certain_length(node.attrib["text"])
                            + " "
                            + node.attrib["class"].split(".")[-1]
                        )
                    elif node.attrib["content-desc"] != "":
                        node.attrib["name"] = (
                            get_words_in_certain_length(node.attrib["content-desc"])
                            + " "
                            + node.attrib["class"].split(".")[-1]
                        )
                    else:
                    ...
                    if node.attrib["class"].split(".")[-1] in [
                        "View",
                        "FrameLayout",
                        "LinearLayout",
                        "RelativeLayout",
                ...
                if (
                    parent.tag == "node"
                    and "material_clock_face" in parent.attrib["resource-id"]
                ):
            ...
            if "xpath1" in parent.attrib and "xpath2" in parent.attrib:
                if parent.attrib["xpath1"] != "//" and parent.attrib["xpath2"] != "//":
                    if node.tag in self.node_to_xpath:
                        ...
                        self.node_to_xpath[node.tag][2] = [
                            parent.attrib["xpath1"],
                            parent.attrib["xpath2"],
                        ]
    ...
    def mid_order_merge(self, node):
        ...
        if (
            self.use_bounds
            and node.tag == "node"
            and self.can_merge_bounds(node.attrib["bounds"], node.attrib["bounds"])
        ):
            can_merge = True
        if (
            self.merge_switch
            and node.tag == "node"
            and node.attrib["checked"] == "true"
        ):
            node.attrib["func-desc"] = (
                ", it has a switch and the switch is currently on,"
            )
            can_merge = True
        if (
            self.merge_switch
            and node.tag == "node"
            and node.attrib["checkable"] == "true"
            and node.attrib["checked"] == "false"
        ):
            node.attrib["func-desc"] = (
                ", it has a switch and the switch is currently off,"
            )
            can_merge = True
        ...
        if can_merge:
            ...
            if parent is not None and parent.tag == "node":
                ...
                for i, v in enumerate(children):
                    parent.insert(index + i, v)
                parent.attrib["func-desc"] = self.merge_description(
                    parent.attrib["func-desc"], node.attrib["func-desc"]
                )
                parent.remove(node)
    ...
    def get_recycle_nodes(self, root):
        ...
        for element in root.iter():
            if (
                "scrollable" in element.attrib
                and element.attrib["scrollable"] == "true"
            ):
    ...
    def same_subtree(self, tree1, tree2):
        if (
            tree1.attrib["class"] != tree2.attrib["class"]
            or tree1.attrib["resource-id"] != tree2.attrib["resource-id"]
        ...
        for element in self.root.iter():
            if (
                ...
                for node in recycle_nodes:
                    if (
                        element.attrib["class"] == node.attrib["class"]
                        and element.attrib["resource-id"] == node.attrib["resource-id"]
                        and element.attrib["func-desc"] == node.attrib["func-desc"]
                    ):
                        for child in list(node):
