    ...
    def AguvisActionSpace_mapping(self, response):
        ...
        if "pyautogui" in response:
            ...
            if match:
                ...
                if action == "write":
                    write_match = re.search(r"pyautogui\.write\(message=['\"](.+?)['\"]\)", response)
                    if write_match:
                        message = write_match.group(1)
                        Action = f"do(action=\"Type\", text=\"{message}\")"
                elif action == "click":
                    click_match = re.search(r"pyautogui\.click\(x=(\d*\.?\d*),\s*y=(\d*\.?\d*)\)", response)
                    if click_match:
            ...
            if match:
                action = match.group(1).lower()
                if action == "swipe":
                    swipe_match = re.search(r"mobile\.swipe\(from_coord=\[(\d*\.?\d*),\s*(\d*\.?\d*)\],\s*to_coord=\[(\d*\.?\d*),\s*(\d*\.?\d*)\]\)", response)
                    if swipe_match:
                    ...
                    long_press_match = re.search(r"mobile\.long_press\(x=(\d*\.?\d*),\s*y=(\d*\.?\d*)\)", response)
                    if long_press_match:
                        x = long_press_match.group(1)
                        y = long_press_match.group(2)
                        Action = f"do(action=\"Long Press\", element=[{x}, {y}])"
                elif action == "terminate":
                    terminate_match = re.search(r"mobile\.terminate\(status=['\"](.+?)['\"]\)", response)
                    if terminate_match:
            ...
            terminate_match = re.search(r"terminate\(status=['\"](.+?)['\"]\)", response)
            if terminate_match:
                status = terminate_match.group(1)
                Action = f"finish(message=\"{status}\")"
