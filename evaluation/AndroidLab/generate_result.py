...
def find_all_task_files(all_task_config_path) -> List[str]:
    ...
    for task in all_task_config_path:
        if isdir(task):
            tasks += [
                relpath(path, ".")
                for path in glob(join(task, "**/*.yaml"), recursive=True)
            ]
        elif isfile(task):
            tasks.append(task)
        else:
...
def evaluate_input_dir(input_dir, task_yamls, create_time, args):
    ...
    for app_task_config_path in task_files:
        ...
        print(
            f"    Evaluation_Task '{app_task.name}' loaded from config {app_task_config_path}"
        )
...
def parse_args():
    ...
    group.add_argument("--output_folder", type=str, default="./logs/evaluation/")
    group.add_argument(
        "--output_excel", type=str, default="./logs/evaluation/test_name.xlsx"
    )
    group.add_argument("--total_num", type=int, default=138)
    group.add_argument("--judge_model", type=str, default="gpt-4o-2024-05-13")
    group.add_argument("--api_base", type=str, default="https://api.openai.com/v1")
    group.add_argument("--api_key", type=str, default="sk-")
...
def main():
    args = parse_args()
    assert args.judge_model in [
        "glm4",
        "gpt-4o-2024-05-13",
    ], "We only support glm4 or gpt-4o for judge model"
    ...
    input_dirs = [
        os.path.join(input_folder, input_dir) for input_dir in os.listdir(input_folder)
    ]
