...
class AutoTask:
    ...
    def set_system_prompt(self, instruction):
        self.record.history = [
            {"role": "system", "content": self.agent.system_prompt(instruction)}
        ]
...
class TextOnlyTask(AutoTask):
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_TEXT_GPT
...
class ScreenshotTask(TextOnlyTask):
    ...
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_MLLM_DIRECT
                + f"\n\nTask Instruction: {instruction}",
            }
        ]
...
{self.actions}"""
        ...
        self.page_executor.image_width, self.page_executor.image_height = (
            resized_width,
            resized_height,
        )
    ...
    def set_system_prompt(self, instruction):
        ...
        self.record.history = [
            {"role": "system", "content": SYSTEM_PROMPT_ANDROID_InternVL_Pixellevel}
        ]
...
class CogAgentTask(TextOnlyTask):
    def run_step(self, round_count):
        ...
        prompt = (
            ...
            if round_count == 0
                ...
                {"current_app": self.controller.get_current_app()}, ensure_ascii=False
            )
        )
        try:
            image_path = self.page_executor.current_screenshot
    ...
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_MLLM_CogAgent
                + f"\n\nTask Instruction: {instruction}",
            }
        ]
...
class ScreenshotReactTask(ScreenshotTask):
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_MLLM_DIRECT_REACT
                + f"\n\nTask Instruction: {instruction}",
            }
        ]
...
class ScreenSeeActTask(TextOnlyTask):
    ...
    def set_system_prompt(self, instruction):
        self.record.history = [
            {"role": "system", "content": SeeActPrompts.QUERY_SYSTEM_PROMPT}
        ]
    ...
    def run_step(self, round_count):
        ...
        try:
            ...
            query_message = self.agent.prompt_to_message(
                query_user_prompt, [image_path]
            )
            referring_user_prompt = SeeActPrompts.REFERRING_USER_PROMPT.format(
                option_prompt="\n".join(
                    f"{item['key']} | {item['value']}" for item in choices_list
                )
            )
...
class TextOnlyReactTask(TextOnlyTask):
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_TEXT_ReAct
                + f"\n\nTask Instruction: {instruction}",
            }
        ]
...
class TextOnlyFineTuneTask(TextOnlyTask):
    def set_system_prompt(self, instruction):
        self.record.history = [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_ANDROID_TEXT_GLM_v1_5
                + f"\n\nTask Instruction: {instruction}",
            }
        ]
    ...
    def run_step(self, round_count):
        ...
        try:
            ...
            current_message = {
                "role": "user",
                "content": app_info + compressed_xml_json,
            }
            rsp = self.agent.act([*self.record.history, current_message])
        except Exception as e:
            print_with_color(f"Error: {e}", "red")
