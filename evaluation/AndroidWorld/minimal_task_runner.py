...
def _main() -> None:
  ...
  agent = seeact_v.SeeAct_V(env, infer.Gpt4Wrapper('gpt-4o'),grounding_model_name="internvl3_dynamic", grounding_model_address='http://10.140.66.153:23333/',)
  print('Goal: ' + str(task.goal))
  is_done = False
  for _ in range(50): # Changed to 50 steps
    response = agent.step(task.goal)
    if response.done:
      print('Agent terminated early.')
      is_done = True
      break
  is_done = True
