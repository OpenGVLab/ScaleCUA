...
def _suggest_keyword(typo: str, keywords: list[str], threshold: int = 80) -> str:
    """Suggests a keyword."""
...
def run(
    ...
    def run_episode(task: task_eval.TaskEval) -> episode_runner.EpisodeResult:
        ...
        return episode_runner.run_episode(
            ...
            save_dir=os.path.join(checkpointer.directory, task.name),
        )
...
def _extract_task_metadata() -> pd.DataFrame:
    ...
    return df.set_index(_TASK_TEMPLATE_COLUMN)[["difficulty", "optimal_steps", "tags"]]


def _print_results_by_tag(result_df: pd.DataFrame) -> None:
    exploded_df = result_df.explode("tags").reset_index()
    exploded_df.replace(
        regex={"tags": r""}, value="untagged", inplace=True
...
def process_episodes(
    episodes: list[dict[str, Any]], print_summary: bool = False, checkpoint_dir="."
) -> pd.DataFrame:
    ...
    result_df = df.groupby(constants.EpisodeConstants.TASK_TEMPLATE, dropna=True).agg(
        {
    ...
    if print_summary:
        ...
        result_file = os.path.join(output_dir, f"result_{timestamp}.csv")
        result.to_csv(result_file, index=True)
        print(f"Result saved to {result_file}")

        tags_file = os.path.join(output_dir, f"tags_{timestamp}.csv")
        tags_df.to_csv(tags_file, index=True)
