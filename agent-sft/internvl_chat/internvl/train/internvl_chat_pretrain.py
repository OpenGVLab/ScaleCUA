...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        data_dir = os.path.join(
            local_tmp_dir,
            meta["annotation"].replace("/", "_").replace(":", "-"),
            f"{basename}_temp",
        )
        ...
        if os.path.exists(temp_path):
            ...
            self.raw_data = self.load_json(temp_path)
        else:
            # If it does not exist, read the raw data from the original annotation file
            # with open(meta['annotation'], 'r') as f:
            #     self.raw_data = f.readlines()
            self.raw_data = self.load_json(meta["annotation"])
            ...
            start_line = min(lines_per_rank * current_rank, total_lines - 1)
                    ...
                    with open(temp_path, "w") as f:
                        raw_data_str = [
                            (
                                item + "\n"
                                if type(item) == str
                                else json.dumps(item).decode("utf-8") + "\n"
                            )
                            for item in self.raw_data
                        ]
                        # if len(raw_data_str) > 0 and len(raw_data_str[-1]) > 0:
                        raw_data_str[-1] = raw_data_str[-1][:-1]
                        f.writelines(raw_data_str)
                except Exception as e:
                    print(
                        f"Failed to write to {temp_path}. Source: {meta['annotation']}. Exception:",
                        e,
                    )
                    raise ValueError(
                        f"Failed to write to {temp_path}. Source: {meta['annotation']}."
                    )
            ...
            if path.endswith(".jsonl"):
                if "s3://" in path:
                    ...
                    with open(path, "r") as f:
                        items = f.readlines()
                data_list = [line for line in items if line.strip() != ""]
    ...
    def get_preprocess_function(self):
        ...
        if self.template_name == "Hermes-2":
            ...
            preprocess_function = preprocess_internvl2_5_private_v1
        elif self.template_name in GUI_CONV_TEMPLATE:
            preprocess_function = preprocess_gui_conv
        else:
    ...
    def multi_modal_get_item(self, data_item):
        ...
        idx = 0
        while data_item["conversations"][idx]["from"] != "human":
            idx += 1

        if "<image>" not in data_item["conversations"][idx]["value"]:
            data_item["conversations"][idx]["value"] = (
    ...
    def video_get_item(self, data_item):
        ...
        if "<video>" not in data_item["conversations"][idx]["value"]:
            data_item["conversations"][idx]["value"] = (
                "<video>\n" + data_item["conversations"][0]["value"]
            )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            if try_cnt > max_try:
                raise StopIteration
            try:
                if isinstance(self.raw_data[i], str):
                    self.raw_data[i] = json.loads(self.raw_data[i])
                ...
                if not isinstance(e, (UnidentifiedImageError, FileNotFoundError)):
                    traceback.print_exc()
                    print(
                        f"Failed to load data in {self.ds_name}: {self.raw_data[i]}",
                        flush=True,
                    )
                if isinstance(self.raw_data[i], str):
                    self.raw_data[i] = json.loads(self.raw_data[i])
...
def build_datasets(
    ...
    if type(ds_collections) == list:
        for item in ds_collections:
            new_ds_collections.update(json.loads(open(item, "r").read()))
        ds_collections = new_ds_collections

    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        conv_style = ds_collections[ds_name].get("conv_style", None)
        conv_style = data_args.conv_style if conv_style is None else conv_style
        repeat_time = ds_collections[ds_name].get("repeat_time", 1)
        if repeat_time == 0:
            logger.info(f"Skipping {ds_name} due to repeat_time=0")
            continue
        split_anno_by_rank = ds_collections[ds_name].get("split_anno_by_rank", True)
        ...
        dataset = LazySupervisedDataset(
            ...
            # hyperparameters for packed training
            split_anno_by_rank=split_anno_by_rank,
